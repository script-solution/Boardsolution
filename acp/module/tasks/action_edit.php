...
final class BS_ACP_Action_tasks_edit extends BS_ACP_Action_Base
{
	public function perform_action($type = 'edit')
	{
		...
		if($type == 'edit')
		{
			...
			// does the task exist?
			$task = $this->cache->get_cache('tasks')->get_element($id);
			if($task === null)
		...
		$title = $this->input->get_var('task_title','post',PLIB_Input::STRING);
		$file = $this->input->get_var('task_file','post',PLIB_Input::STRING);
		$interval = $this->input->get_var('interval','post',PLIB_Input::INTEGER);
		$interval_type = $this->input->correct_var(
			'interval_type','post',PLIB_Input::STRING,array('days','hours','minutes'),'days'
		);
		$time_hour = $this->input->get_var('time_hour','post',PLIB_Input::INTEGER);
		$time_min = $this->input->get_var('time_min','post',PLIB_Input::INTEGER);
		$time_sec = $this->input->get_var('time_sec','post',PLIB_Input::INTEGER);
		if($time_hour !== null && $time_min !== null && $time_sec !== null)
			$time = $time_hour.':'.$time_min.':'.$time_sec;
		else
			$time = null;
		$enabled = $this->input->get_var('enabled','post',PLIB_Input::INT_BOOL);
		...
		$file = basename($file);
		if(!$is_default && !is_file(PLIB_Path::inner().'src/tasks/'.$file))
		...
		$this->set_success_msg($this->locale->lang('task_'.$type.'_success'));
