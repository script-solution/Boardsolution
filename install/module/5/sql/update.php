...
final class BS_Install_Module_5_SQL_Update extends BS_Install_Module_5_SQL_Base
{
	...
	protected function run()
	{
		...
		$db->execute("INSERT INTO `{$consts['BS_TB_BBCODES']}`
			...
			(16, 'topic', 'link', 'text', '', '<a target=\"_blank\" href=\"{BSF}action=redirect&amp;loc=show_topic&amp;tid={PARAM}\">{TEXT}</a>', 'required', 'text', 0, 0, 0, 'inline'),
			(17, 'post', 'link', 'text', '', '<a target=\"_blank\" href=\"{BSF}action=redirect&amp;loc=show_post&amp;id={PARAM}\">{TEXT}</a>', 'required', 'text', 0, 0, 0, 'inline'),
		...
		include_once(FWS_Path::server_app().'config/community.php');
