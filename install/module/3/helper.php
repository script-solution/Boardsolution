...
final class BS_Install_Module_3_Helper extends FWS_UtilBase
{
	...
	public static function collect_vals()
	{
		...
		if($host != '' && $login != ''  && $database != '')
		{
			...
			{
				$check['mysql_select_db_failed'] = $ex->get_mysql_error();
			}
			catch(FWS_DB_Exception_ConnectionFailed $ex)
			{
				$check['mysql_connect_failed'] = $ex->get_mysql_error();
			}
		}
	...
	public static function check($values,&$status)
	{
		...
		{
			if(($key == 'mysql_select_db_failed' || $key == 'mysql_connect_failed') && is_string($value))
				$errors[] = sprintf($locale->lang('error_'.$key), $value);
			elseif($key != 'gd' && $key != 'chmod_themes' && !$value)
