 ...
 * 
 * @version			$Id: module_topics.php 43 2008-07-30 10:47:55Z nasmussen $
...
final class BS_Front_Module_topics extends BS_Front_Module
{
	...
	public function run()
	{
		...
		if($forums->get_forum_type($fid) == 'contains_threads')
		{
			...
			{
				$subscribe_forum_url = $url->get_url(
			...
			if($input->isset_var(BS_URL_ORDER,'get'))
			{
				...
				$page_url = $url->get_url(0,$params);
			}
			else
				$page_url = $url->get_topics_url($fid,'&amp;','{d}');
